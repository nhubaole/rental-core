...
const getReturnRequestByTenantID = `-- name: GetReturnRequestByTenantID :many
SELECT rr.id, rr.contract_id, r.id as room_id, rr.reason, rr.return_date, rr.status, rr.deduct_amount, rr.total_return_deposit, rr.created_user, rr.created_at, rr.updated_at
FROM public.return_requests rr LEFT JOIN public.contracts c
ON rr.contract_id = c.id
LEFT JOIN public.rooms r ON c.room_id = r.id
...
type GetReturnRequestByTenantIDRow struct {
	ID                 int32            `json:"id"`
	ContractID         *int32           `json:"contract_id"`
	RoomID             *int32           `json:"room_id"`
	Reason             *string          `json:"reason"`
...
func (q *Queries) GetReturnRequestByTenantID(ctx context.Context, createdUser *int32) ([]GetReturnRequestByTenantIDRow, error) {
	rows, err := q.db.Query(ctx, getReturnRequestByTenantID, createdUser)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetReturnRequestByTenantIDRow
	for rows.Next() {
		var i GetReturnRequestByTenantIDRow
		if err := rows.Scan(
			&i.ID,
			&i.ContractID,
			&i.RoomID,
